...
const Ventas = {
  ...
  async startCamera() {
    ...
    try {
      // Resolución moderada: 1920x1080 puede bloquear readyState en mobile
      const constraints = {
        video: {
  ...
  startCameraScan(video) {
    ...
    try {
      ...
      this._scanInterval = setInterval(() => {
        // readyState >= 2 (HAVE_CURRENT_DATA) es suficiente para dibujar
        ...
        try {
          ...
          // ZXing RGBLuminanceSource necesita RGB (3 canales), no RGBA (4 canales)
          // Hay que convertir manualmente de RGBA a RGB
          ...
          const luminance = new ZXing.RGBLuminanceSource(rgb, w, h);
          ...
          if (result) {
            ...
            // Parar el interval ANTES de cerrar la cámara y procesar
            clearInterval(this._scanInterval);
            this._scanInterval = null;

            this.stopCamera();
            // Pequeño delay para que stopCamera termine antes de abrir modal
            setTimeout(() => this.addByCode(code), 80);
          }
        } catch (e) {
          // NotFoundException es normal — no hay código en el frame
          // Otros errores los ignoramos para no romper el loop
        }
      }, 200);

    } catch (e) {
      console.error('ZXing scan error:', e);
      if (status) status.textContent = 'Error al iniciar el lector.';
    }
  },
